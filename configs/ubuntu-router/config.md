# 🟦 Ubuntu Router — Guía de Configuración
 
> **Host:** `ubuntu-router` · **IP Gateway:** `192.168.10.1 / 192.168.20.1 / 192.168.30.1` · **SO:** Ubuntu Server 22.04 LTS
...
Archivo: `/etc/suricata/suricata.yaml` (extracto relevante)
...
Archivo: `/etc/suricata/threshold.config`
...
| Fichero | Contenido |
|---|---|
| `/var/log/suricata/eve.json` | Logs completos en formato JSON (integración con Wazuh) |
| `/var/log/suricata/fast.log` | Alertas en formato legible por humanos |
 
```bash
# Monitorizar alertas en tiempo real
sudo tail -f /var/log/suricata/fast.log
 
# Ver eventos JSON (consumidos por Wazuh)
...
```bash
# Comprobar interfaces e IPs asignadas
ip addr show
 
# Confirmar que el routing está activo (debe retornar 1)
cat /proc/sys/net/ipv4/ip_forward
 
# Comprobar tabla de rutas
ip route show
 
# Listar reglas nftables activas
